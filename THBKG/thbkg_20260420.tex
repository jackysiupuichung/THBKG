...
baseline, every evaluation pair $(v_t, v_d)$ is assigned to a stratum
along two orthogonal axes, both defined using only evidence dated
before the pair's Phase~II entry year $y$ (i.e.\ respecting the same
decision-aligned cutoff used for features).

\emph{Evidence axis} (three levels), based on which target--disease
edge types exist with snapshot year $< y$:
\begin{itemize}
\item \textit{direct\_evidence} --- at least one edge of a
...
as always present;
\item \textit{literature\_only} --- no direct edge, but at least one
\texttt{literature} (text-mined co-occurrence) edge;
\item \textit{evidence\_free} --- neither; no direct target--disease
evidence of any kind at the decision point. This is the majority class
(75.4\% of Phase~II pairs) and the regime in which a direct-edge model
receives an all-zero feature vector.
\end{itemize}

\emph{Novelty axis} (two levels), based on the target's prior clinical
history:
\begin{itemize}
\item \textit{pioneer} --- the target had not entered Phase~II for any
\emph{other} disease in an earlier year; this is the target's first
appearance in the clinical record;
\item \textit{known} --- the target has previously reached Phase~II in
at least one other disease context, so partial clinical precedent
exists even when direct evidence for this specific indication does not.
\end{itemize}

The two axes are crossed to give six strata
(\{pioneer, known\} $\times$ \{direct\_evidence, literature\_only,
evidence\_free\}); stratified RR@$N$ and classification metrics are
reported per stratum (and per stratum $\times$ therapeutic area) so
that the location of the \eahgt{} advantage --- concentrated in the
\textit{evidence\_free} and \textit{literature\_only} cells, and in
particular for \textit{pioneer} targets --- can be read directly.
...
\caption{Relative risk by top-$N$ cutoff within each evidence /
novelty stratum (Section~\ref{sec:strata}), for all seven experimental
conditions of Table~\ref{tab:conditions} plus RDG and OTS. Top row:
the three evidence-axis strata; bottom row: the two novelty-axis
strata. Stratum $\times$ therapeutic-area breakdowns of the
classification metrics are reported in Supplementary
Figure~\ref{fig:classification_by_stratum_ta}.}
...
Section~\ref{sec:strata}). Among pairs with no prior
...
evidence-free and literature-only strata where direct-edge models have
