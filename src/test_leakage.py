...
def test_leakage():
    ...
    params = {
        ...
        'mode': 'event' # Check logic for event mode first as it's the base
    }
    ...
    train_mask, val_mask, test_mask = get_temporal_masks(
        ...
        params['train_year'], 
        params['val_year']
    )[sup_edge_type] # Returns (train, val, test) tuple for the specific edge type?
    ...
    assert t_train_max <= params['train_year'], f"Train Leakage! Found year {t_train_max} > {params['train_year']}"
    assert t_val_min > params['train_year'], f"Val Leakage! Found year {t_val_min} <= {params['train_year']}"
    assert t_val_max <= params['val_year'], f"Val Leakage! Found year {t_val_max} > {params['val_year']}"
    assert t_test_min > params['val_year'], f"Test Leakage! Found year {t_test_min} <= {params['val_year']}"
    
    print("✅ Temporal Separation Verified.")
    
    # CHECK 2: Context vs Target Leakage
    # train_context (used for Input) is typically filter_graph_by_time(train_year)
    # Target is val_edges
    # Ensure NO val_edges are present in train_context
    
    print("\n   Verifying Input Context Safety...")
    train_context = filter_graph_by_time(data, params['train_year'])
